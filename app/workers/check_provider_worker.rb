class CheckProviderWorker
  ...
  def check_twitter(user, auth)
    ...
    if auth.since_id.nil?
      ...
      auth.update(since_id: lfavorites[0].id)
  ...
  def save_google(auth, save_media_infos)
  ...
  def find_or_create_folder_google(collection, folder_names)
  ...
  def create_folder_google(collection, folder_names)
    folder_name = folder_names.shift
    return collection if folder_name.blank?
